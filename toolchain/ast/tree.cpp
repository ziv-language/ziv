...
AST::Node AST::add_node(NodeKind kind,
...
void AST::add_child(Node parent, Node child) {
    ...
    if (nodes_[child.index_].parent != 0) {
        ...
        old_parent_children.erase(std::remove(old_parent_children.begin(),
...
AST::TreeIterator& AST::TreeIterator::operator++() {
    ...
    while (true) {
        ...
        auto it = std::find(parent.children.begin(), parent.children.end(), current_index);
