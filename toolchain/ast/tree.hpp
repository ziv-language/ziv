...
class AST::TreeIterator
    : public llvm::iterator_facade_base<TreeIterator,  // This is the type we're defining
                                        std::forward_iterator_tag,  // This iterator is a forward
                                                                    // iterator
                                        AST::Node,                  // The type of the elements
                                        ptrdiff_t,  // The type of the difference between two
                                                    // iterators
                                        const AST::Node*,
                                        const AST::Node> {
public:
...
private:
    ...
    TreeIterator(const AST* ast, AST::Node node) : ast_(ast), node_(node) {}
...
class AST::ChildIterator
    : public llvm::iterator_facade_base<ChildIterator,  // This is the type we're defining
                                        std::forward_iterator_tag,  // This iterator is a forward
                                                                    // iterator
                                        AST::Node,                  // The type of the elements
                                        ptrdiff_t,  // The type of the difference between two
...
inline llvm::iterator_range<AST::TreeIterator> AST::get_nodes() const {
    if (nodes_.empty()) {
        return llvm::make_range(TreeIterator(this, Node()), TreeIterator(this, Node()));
    }
    return llvm::make_range(TreeIterator(this, get_root()),
...
inline llvm::iterator_range<AST::TreeIterator> AST::get_subtree(Node node) const {
    ...
    return llvm::make_range(TreeIterator(this, node), TreeIterator(this, Node(nodes_.size())));
}

inline llvm::iterator_range<AST::ChildIterator> AST::get_children(Node node) const {
    if (node.index_ >= nodes_.size()) {
        return llvm::make_range(ChildIterator(), ChildIterator());
    }
    return llvm::make_range(ChildIterator(this, node, 0),
